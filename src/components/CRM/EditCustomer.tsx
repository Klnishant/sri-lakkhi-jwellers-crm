...
export default function EditCustomerModal({
  ...
  const [form, setForm] = useState<ICustomer | null>(customer);
  ...
  const [originalData] = useState(form);
  ...
  const set = (key: keyof ICustomer, val: string) => {
    setForm((prev: ICustomer | null) => ({ ...prev, [key]: val }) as ICustomer);
 ...
 const handleSave = async () => {
  ...
  try {
    const response = await axios.patch("/api/edit-customer", form);
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FieldWrapper label="Anniversary" error={errors.anniversary}>
                <InputField
                  ...
                  placeholder="DD / MM / YYYY"
                  value={
                    form?.anniversary instanceof Date
                      ? form.anniversary.toISOString().split("T")[0]
                      : form?.anniversary
                        ? new Date(form.anniversary).toISOString().split("T")[0]
                        : ""
                  }
